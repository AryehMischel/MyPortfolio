...
let networkSpeedThreshold = 38; //not accurate at all

// src's
let rootPath = "https://d368ik34cg55zg.cloudfront.net/";
let avatarPath = rootPath + "avatarWithAnimation.glb";
let nonInteractiveAvatarPath =
  rootPath + "AryehAvatarNonInteractiveWithEyes.glb";
...
let avatarHeadDissolveShader = null;
...
let tweenHeadDissolveShader = null;

//tween for head animating back to center position
let animateAvatarToOriginTween = null;

//colliders
const animateHeadTriggerRectColl = document.getElementById(
  "head-trigger-collider"
);
let containersOverlappingWithTriggerColl = document.getElementsByClassName(
  "intersects-with-coll"
);
...
function animate(time) {
  ...
  if (interactiveAvatarInScene && !dissolveEffectStarted) {
  ...
  if (interactiveAvatar && animateHead) {
    if (isMobile) {
      if (isMobile && !fingerDown && mobileAnimation && trackBubble) {
        ...
        const relativeX =
          ((triggerCenterX - canvasRect.left) / canvasRect.width) * 2 - 1;
        const relativeY =
          1 - ((triggerCenterY - canvasRect.top) / canvasRect.height) * 2;
        detectMouseOverSphereColl(
          new Vector2(relativeX.toFixed(2), relativeY.toFixed(2))
        );
      }
    }
    rotateHead();
...
function addTorus() {
  ...
  (torus.axis = new Vector3(1.0, 0, 0)), torus.scale.set(0.2, 0.2, 0.2);
...
function loadModels() {
  ...
  loader.load(avatarPath, function (gltf) {
    ...
    interactiveAvatar.traverse((object) => {
      ...
      if (object.name == "avaturn_body002") {
        console.log("found body in interactive avatar");
        avatarBodyBaseMaterial = object.material.clone();
        avatarBodyNode = object;
      }

      if (object.name == "avaturn_body_1") {
        console.log("found eyes in interactive avatar");
        avatarEyesBaseMaterial = object.material.clone();
        avatarEyesNode = object;
      }

      if (object.name == "avaturn_look_0") {
        //
        console.log("found clothes in interactive avatar");
        avatarClothesBaseMaterial = object.material.clone();
        avatarClothesNode = object;
      }

      if (object.name == "avaturn_hair_0") {
        console.log("found hair in interactive avatar");
        avatarHairBaseMaterial = object.material.clone();
        avatarHairNode = object;
      }

      if (object.name == "avaturn_body001") {
        console.log("found heasd 2  in interactive avatar");
        avatarHeadBaseMaterial = object.material.clone();
        avatarHeadNode = object;
      }

      if (object.name == "avaturn_body_3") {
        mouthMesh.push(object);
        object.visible = false;
      }

      if (object.name == "avaturn_body_4") {
        mouthMesh.push(object);
        object.visible = false;
      }

      if (object.name == "avaturn_body_5") {
        mouthMesh.push(object);
        object.visible = false;
      }

      if (object.name == "avaturn_body003") {
        mouthMesh.push(object);
        object.visible = false;
      }

      if (object.name == "avaturn_body004") {
        mouthMesh.push(object);
        object.visible = false;
      }

      if (object.name == "avaturn_body005") {
        mouthMesh.push(object);
        object.visible = false;
      }

      if (object.name == "avaturn_body006") {
        mouthMesh.push(object);
        object.visible = false;
      }

      if (object.name == "avaturn_body007") {
        mouthMesh.push(object);
        object.visible = false;
      }

      if (object.name == "avaturn_body008") {
        mouthMesh.push(object);
        object.visible = false;
      }

      // if (object.name == "avaturn_body013") {
      //   console.log("found body in interactive avatar")
      //   avatarBodyBaseMaterial = object.material.clone()
      //   avatarBodyNode = object;
      // }
      ...
      // if (object.name == "avaturn_body013") {
      //   console.log("found hair in interactive avatar 12")
      //   avatarHairBaseMaterial = object.material.clone()
      //   avatarHairNode = object;
      // }
    ...
    loader.load(avatarEyeModelPath, function (gltf) {
      ...
      setVisibleInSeconds(eyeModel, 500);
    });
  });
}
...
function createSiluetteMesh() {
  ...
  const avatarSiluetteTexture = textureLoader.load(
    rootPath + "silluetteNoBackground.png"
  );
...
function createDissolveShaders() {
  ...
  avatarClothesDissolveShader = new CustomShaderMaterial({
    ...
    uniforms: {
      ...
      time: { value: 0.2 },
      threshold: { value: 2.0 },
      edgeColor: { value: new Color().setHex(0xd1ecff) }, // Add a uniform for the color
  ...
  avatarHeadDissolveShader = new CustomShaderMaterial({
    baseMaterial: avatarHeadBaseMaterial,
    uniforms: {
      growFade: { value: false },
      brightness: { value: 10.0 },
      time: { value: 0.2 },
  ...
  avatarHeadNode.material = avatarHeadDissolveShader;
}
...
function animateHeadIn() {
  tweenHeadDissolveShader = new TWEEN.Tween({ x: 1 })
    .to({ x: 0 }, 1000)
    .onComplete(() => {})
    .onUpdate((object) => {
      avatarHeadDissolveShader.uniforms.threshold.value = object.x;
    });
  tweenHeadDissolveShader.start();
}
...
function updateDissolveShaders() {
  ...
  if (tweenHeadDissolveShader) {
    tweenHeadDissolveShader.update();
  }
...
async function setupInteractiveAvatar() {
  interactiveAvatarLoaded = true;
  ...
  if (tweenHeadDissolveShader.isPlaying()) {
    tweenHeadDissolveShader.stop();
  }
  tweenBodyDissolveShader = null;
  tweenClothesDissolveShader = null;
  tweenEyesDissolveShader = null;
  tweenHairDissolveShader = null;
  tweenHeadDissolveShader = null;
  // action.play();
  animateHead = true;
  startBlinking();
  action.play();

  for (let i = 0; i < mouthMesh.length; i++) {
    mouthMesh[i].visible = true;
  }
...
function animateAvatarToOrigin() {
  ...
  let mouse2 = {
    u: previousMouseIntersectionPoint.x,
    v: previousMouseIntersectionPoint.y,
  };
  let center = { u: 0, v: 0 };
...
for (var i = 0; i < containersOverlappingWithTriggerColl.length; i++) {
  containersOverlappingWithTriggerColl[i].addEventListener(
    "mousemove",
    (event) => {
...
function estimateNetworkSpeed() {
  ...
  function displaySpeed() {
    ...
    if (speedInMbps > networkSpeedThreshold) {
      ...
      setTimeout(() => {
        addTorus();
      }, 1); //timeout to insure this is queued up to run on the next event loop
...
function rotateHead() {
  ...
  let [yaw, pitch] = calculateYawAndPitchDifference(
    headBoneHelper.rotation,
    headBoneOriginHelper.rotation
  );
  const targetQuaternion = calculateTargetQuaternion(neckBone, targetPosition);
  const correctionQuaternion = new Quaternion();
  correctionQuaternion.setFromAxisAngle(new Vector3(0, 1, 0), Math.PI); // Rotate 180 degrees around the Y-axis
  const correctedTargetQuaternion = targetQuaternion
    .clone()
    .multiply(correctionQuaternion);
  headBone.quaternion.slerp(correctedTargetQuaternion, 0.02);

  if (
    true ||
    Math.abs(MathUtils.radToDeg(yaw)) > 6 ||
    Math.abs(MathUtils.radToDeg(pitch)) > 4
  ) {
    ...
    const finalTargetQuaternion =
      correctedTargetQuaternion.multiply(offsetQuaternion);
    const halfwayQuaternion = new Quaternion()
      .copy(neckBoneOriginHelper.quaternion)
      .slerp(finalTargetQuaternion, 0.35);
    neckBone.quaternion.slerp(halfwayQuaternion, 0.03);
  }
}

function playDissolveShader() {
  console.log("playing dissolve");
  animateDissolveShaders();
  dissolveShaderPlaying = true;
  dissolveEffectStarted = true;
}
window.playDissolveShader = playDissolveShader;
